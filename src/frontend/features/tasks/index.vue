<template>
    <div class="w-full h-full text-green-400 font-mono text-xs flex flex-col">
        <div class="p-4 shrink-0">
            <h2 class="text-lg text-orange-400">Генератор событий</h2>
            <p class="text-sm">Модуль генерации событий. Получить событие можно командой <b class="text-orange-400">!событие</b></p>
        </div>
...
const actions = ref([
    ...
    {
        id: 'show-last-events',
        label: 'Показать последние события',
        handler: async () => {
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
    }
